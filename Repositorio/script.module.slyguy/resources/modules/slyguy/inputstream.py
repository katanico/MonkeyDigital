...
from .log import log
from .constants import IA_ADDON_ID, IA_VERSION_KEY, IA_HLS_MIN_VER, IA_MPD_MIN_VER, IA_MODULES_URL, IA_CHECK_EVERY, COMMON_ADDON_ID, KODI_VERSION
from .language import _
from .util import md5sum, remove_file, get_system_arch, hash_6, kodi_rpc
...
class HLS(InputstreamItem):
    ...
    def __init__(self, force=False, live=True):
        self.force = force
        self.live  = live
    ...
    def do_check(self):
        ...
        return (self.force or (self.live and hls_live) or (not self.live and hls_vod)) and supports_hls()
...
def get_ia_addon(required=False, install=False):
    try:
        try: return xbmcaddon.Addon(IA_ADDON_ID)
        except: pass

        if install:
            xbmc.executebuiltin('InstallAddon({})'.format(IA_ADDON_ID), True)
            
        kodi_rpc('Addons.SetAddonEnabled', {'addonid': IA_ADDON_ID, 'enabled': True}, raise_on_error=True)
...
def open_settings():
    ia_addon = get_ia_addon(required=True, install=True)
...
def supports_hls():
    return require_version(IA_HLS_MIN_VER)

def supports_mpd():
    return require_version(IA_MPD_MIN_VER)

def supports_playready():
    ia_addon = get_ia_addon()
    return bool(ia_addon and KODI_VERSION > 17 and xbmc.getCondVisibility('system.platform.android'))
